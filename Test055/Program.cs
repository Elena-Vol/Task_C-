...
if (matrixNumbers.GetLength(0) != matrixNumbers.GetLength(1)) { Console.WriteLine($"Замена невозможна"); }
else
{     // int[,] matrixChange = ChangeRowsOnColumns(matrixNumbers); 
    ...
    ChangeRowsOnColumns(matrixNumbers); Console.WriteLine($"После перемены строк и колонок - \n{PrintMatrix(matrixNumbers)}");
}
